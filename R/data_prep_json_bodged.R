...
#unnest_longer and unnest_wider NOT WORKING not working with current packages
#tidyr v. 1.0.0 required but for some reason won't load here. 
#anyway, there must be a more straightforward way of rectangling this json file...
#in the meantime I'll just load previously saved version of `regions`
...
#bounding boxes
boxinfo <- regions %>% 
  mutate(x_right = left + width - 1,
         y_bottom = top + height - 1) %>%  
